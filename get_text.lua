...
function getText:get_back_descriptions()
    local backs = {}
    for back_id, back in pairs(G.P_CENTERS) do
        if back.set ~= "Back" then goto continue end
        if not table.any(ALLOWED_DECKS, function(check) return check == back.name end) then goto continue end
        local loc_lookup = Back_Loc[back_id]
        local loc_args = {}
        local loc_nodes = {}
        local description = ""
        if type(loc_lookup) == "table" then
            for _, v in ipairs(loc_lookup) do
                table.insert(loc_args, back.config[v])
            end
        elseif type(loc_lookup) == "function" then
            loc_args = loc_lookup(back)
        else
            sendErrorMessage("Could not find localization arguments for back " .. back_id)
        end
        localize { type = 'descriptions', key = back.key, set = 'Back', nodes = loc_nodes, vars = loc_args }
...
local function get_edition_args(name, card_config)
    local loc_args = {}
    if name == "foil" then
        loc_args = { card_config.config.extra }
...
local function get_enhancements_args(name, card_config)
    ...
    if name == "Bonus Card" then
        loc_args = { card_config.config.bonus }
    elseif name == "Mult Card" then
        loc_args = { card_config.config.mult }
    elseif name == "Wild Card" then
        loc_args = { card_config.config.extra }                           --  localize
    elseif name == "Glass Card" then
        loc_args = { card_config.config.Xmult, card_config.config.extra } -- 2x 1 in 4 chance to break (this is sent as 4)
    elseif name == "Steel Card" then
        loc_args = { card_config.config.h_x_mult }                        -- 1.5 while in hand
    elseif name == "Stone Card" then
        loc_args = { card_config.config.bonus }                           -- is always 50
    elseif name == "Gold Card" then
        loc_args = { card_config.config.h_dollars }                       -- three in hand
    elseif name == "Lucky Card" then
        loc_args = { card_config.config.p_dollars, card_config.config.mult }
    end -- 1 in 5 chance for +20 mult and 1 in 15 for $20 (these are both sent as 20)
    return loc_args
end
        ...
        if not card.ability.effect then
            ...
            for _, v in pairs(G.P_CENTER_POOLS.Enhanced) do
                ...
                if v.loc_txt and type(v.loc_vars) == 'function' then
                    local res = v:loc_vars(nil, card) or
                        {} -- makes twins card work and glorp still works so I think its fine
